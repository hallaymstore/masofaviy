...
async function getDb(){if(db)return db;if(connecting)return connecting;const uri=process.env.MONGODB_URI||'';if(!uri)throw new Error('db_required');connecting=(async()=>{mongoClient=new MongoClient(uri,{serverSelectionTimeoutMS:8000,maxPoolSize:6,minPoolSize:0,maxIdleTimeMS:60000,retryReads:true,retryWrites:true});await mongoClient.connect();db=mongoClient.db(process.env.DB_NAME||'masofaviy');await Promise.all([db.collection('recordings').createIndex({lessonId:1,createdAt:-1}),db.collection('recordings').createIndex({group:1,status:1,createdAt:-1})]).catch(()=>{});return db})();try{return await connecting}finally{connecting=null}}
...
function canWatch(u,r){return managers.has(u.role)||(u.role==='teacher'&&r.teacherUsername===u.username)||(u.role==='student'&&Boolean(u.group)&&r.group===u.group)}
...
async function safeUnlink(p){try{await fsp.unlink(p)}catch(e){if(e.code!=='ENOENT')throw e}}
function install(app){
  // Recordings are intentionally NOT exposed with express.static. Every replay
  // is authorized against the current user and recording group/teacher first.
  app.get('/recordings/:id.webm',async(req,res)=>{try{const u=await user(req),d=await getDb(),id=clean(req.params.id,120),r=await d.collection('recordings').findOne({id,status:'ready'},{projection:{_id:0}});if(!r)throw Object.assign(new Error('recording_not_found'),{status:404});if(!canWatch(u,r))throw Object.assign(new Error('recording_access_denied'),{status:403});const p=finalPath(r.id);if(!fs.existsSync(p))throw Object.assign(new Error('recording_file_missing'),{status:404});res.set('Cache-Control','private, max-age=3600');res.set('Content-Type',r.mimeType||'video/webm');res.sendFile(p)}catch(e){fail(res,e)}});
  app.post('/api/recordings/start/:lessonId',json,async(req,res)=>{try{const u=await user(req),d=await getDb(),l=await lessonFor(d,req.params.lessonId);if(!l)throw Object.assign(new Error('lesson_not_found'),{status:404});if(!canHost(u,l))throw Object.assign(new Error('forbidden'),{status:403});const id=`REC-${crypto.randomUUID()}`;await fsp.writeFile(partPath(id),Buffer.alloc(0),{flag:'wx'});const doc={id,lessonId:l.id,subject:l.subject||'',group:l.group||'',teacherUsername:l.teacherUsername||u.username,teacher:l.teacher||u.name||u.username,status:'recording',mimeType:clean(req.body?.mimeType,120)||'video/webm',createdAt:new Date(),createdBy:u.username,bytes:0};await d.collection('recordings').insertOne(doc);res.status(201).json({ok:true,id})}catch(e){fail(res,e)}});
  app.post('/api/recordings/chunk/:id',raw,async(req,res)=>{try{const u=await user(req),d=await getDb(),r=await d.collection('recordings').findOne({id:clean(req.params.id,120)});if(!r||r.status!=='recording')throw Object.assign(new Error('recording_not_found'),{status:404});const l=await lessonFor(d,r.lessonId);if(!canHost(u,l))throw Object.assign(new Error('forbidden'),{status:403});if(!Buffer.isBuffer(req.body)||!req.body.length)return res.json({ok:true,bytes:0});await fsp.appendFile(partPath(r.id),req.body);await d.collection('recordings').updateOne({id:r.id},{$inc:{bytes:req.body.length},$set:{updatedAt:new Date()}});res.json({ok:true,bytes:req.body.length})}catch(e){fail(res,e)}});
  app.post('/api/recordings/finish/:id',json,async(req,res)=>{try{const u=await user(req),d=await getDb(),r=await d.collection('recordings').findOne({id:clean(req.params.id,120)});if(!r)throw Object.assign(new Error('recording_not_found'),{status:404});const l=await lessonFor(d,r.lessonId);if(!canHost(u,l))throw Object.assign(new Error('forbidden'),{status:403});const src=partPath(r.id),dst=finalPath(r.id);if(fs.existsSync(src))await fsp.rename(src,dst);const size=fs.existsSync(dst)?(await fsp.stat(dst)).size:0;const patch={status:'ready',url:`/recordings/${encodeURIComponent(r.id)}.webm`,bytes:size,durationSec:Math.max(0,Number(req.body?.durationSec)||0),finishedAt:new Date(),updatedAt:new Date()};await d.collection('recordings').updateOne({id:r.id},{$set:patch});res.json({ok:true,item:{id:r.id,lessonId:r.lessonId,...patch}})}catch(e){fail(res,e)}});
  app.get('/api/recordings',async(req,res)=>{try{const u=await user(req),d=await getDb();let q={status:'ready'};if(u.role==='student')q.group=u.group||'__NO_GROUP__';else if(u.role==='teacher')q.teacherUsername=u.username;const items=await d.collection('recordings').find(q,{projection:{_id:0}}).sort({createdAt:-1}).limit(100).toArray();res.set('Cache-Control','no-store');res.json({ok:true,items})}catch(e){fail(res,e)}});
  app.delete('/api/recordings/:id',async(req,res)=>{try{const u=await user(req),d=await getDb(),r=await d.collection('recordings').findOne({id:clean(req.params.id,120)});if(!r)throw Object.assign(new Error('not_found'),{status:404});if(!canWatch(u,r)||u.role==='student')throw Object.assign(new Error('forbidden'),{status:403});await safeUnlink(partPath(r.id));await safeUnlink(finalPath(r.id));await d.collection('recordings').deleteOne({id:r.id});res.json({ok:true})}catch(e){fail(res,e)}});
}
